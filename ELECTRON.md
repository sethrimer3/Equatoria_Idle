...
This runs `npm run build:desktop` first, then opens the built `dist/index.html` in Electron. The Electron shell lives in `electron/main.cjs`, uses `contextIsolation: true`, and leaves `nodeIntegration` disabled. No preload API is exposed.

Electron uses Chromium localStorage for saves. Desktop saves are separate from browser saves because Electron runs under its own app profile. No save migration is performed.
