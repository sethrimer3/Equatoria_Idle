...
export function createRpgRender(container: HTMLElement, rpgSimState: RpgSimState, options: RpgRenderOptions = {}): RpgRender {
  ...
  function updateSapphireLasers(deltaMs: number): void {
    const dt = Math.min(deltaMs / TARGET_FRAME_MS, 3);
    const weaponId = findEquippedWeaponIdByEffect('sapphireShip');
    statsPanel.withDamageSource(weaponId, () => {
  ...
  return {
    ...
    update(deltaMs: number, autoMoveEnabled = false): void {
      ...
      statsPanel.withDamageSource(findEquippedWeaponIdByEffect('gatling'), () => updateSandProjectiles(deltaMs));
      // Update chain whip for all equipped chainWhip weapons
      for (const weaponId of getEffectiveEquippedIds()) {
        const wd = WEAPON_BY_ID.get(weaponId);
        if (wd?.stats.effect?.kind === 'chainWhip') statsPanel.withDamageSource(weaponId, () => updateChainWhip(weaponId, deltaMs));
      }
      // Update vortex and sword combo systems
      for (const weaponId of getEffectiveEquippedIds()) {
        const wd = WEAPON_BY_ID.get(weaponId);
        if (wd?.stats.effect?.kind === 'vortex')    statsPanel.withDamageSource(weaponId, () => updateVortexWeapon(weaponId, deltaMs));
        if (wd?.stats.effect?.kind === 'swordCombo') statsPanel.withDamageSource(weaponId, () => updateSwordCombo(weaponId, deltaMs));
      }
      statsPanel.withDamageSource(findEquippedWeaponIdByEffect('vortex'), () => updateVortexes(deltaMs));
      ...
      statsPanel.withDamageSource(findEquippedWeaponIdByEffect('sunstoneMine'), () => updateSunstoneMines(deltaMs));
