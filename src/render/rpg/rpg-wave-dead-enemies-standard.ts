...
export function sweepStandardDeadEnemies(
  ...
  for (let i = nullstoneEnemies.length - 1; i >= 0; i--) {
    if (nullstoneEnemies[i].hp <= 0) {
      pushDyingEnemy(nullstoneEnemies[i].x, nullstoneEnemies[i].y, FLUID_NULLSTONE_R, FLUID_NULLSTONE_G, FLUID_NULLSTONE_B, 8);
  ...
  for (let i = polyominoEnemies.length - 1; i >= 0; i--) {
    if (polyominoEnemies[i].hp <= 0) {
      pushDyingEnemy(polyominoEnemies[i].x, polyominoEnemies[i].y, FLUID_PROC_R, FLUID_PROC_G, FLUID_PROC_B, 5);
  ...
  for (let i = eyeStalkEnemies.length - 1; i >= 0; i--) {
    if (eyeStalkEnemies[i].hp <= 0) {
      pushDyingEnemy(eyeStalkEnemies[i].x, eyeStalkEnemies[i].y, FLUID_PROC_R, FLUID_PROC_G, FLUID_PROC_B, 5);
  ...
  for (let i = quartzFishEnemies.length - 1; i >= 0; i--) {
    if (quartzFishEnemies[i].hp <= 0) {
      pushDyingEnemy(quartzFishEnemies[i].x, quartzFishEnemies[i].y, FLUID_PROC_R, FLUID_PROC_G, FLUID_PROC_B, 5);
  ...
  for (let i = diamondFishEnemies.length - 1; i >= 0; i--) {
    if (diamondFishEnemies[i].hp <= 0) {
      pushDyingEnemy(diamondFishEnemies[i].x, diamondFishEnemies[i].y, FLUID_PROC_R, FLUID_PROC_G, FLUID_PROC_B, 6);
