...
function drawCubePointNode(c2d: CanvasRenderingContext2D, e: NadirCubePointEnemy): void {
  ...
  if (!isLowGraphics) {
    c2d.shadowBlur = 18;
    c2d.shadowColor = hitFlashMs > 0 ? '#ffffff' : NADIR_NODE_GLOW;
  }

  const coreSize = r * 0.55;
  c2d.fillStyle = hitFlashMs > 0 ? '#ffffff' : NADIR_NODE_COLOR;
  c2d.fillRect(x - coreSize, y - coreSize, coreSize * 2, coreSize * 2);

  c2d.strokeStyle = hitFlashMs > 0 ? '#ffffff' : NADIR_NODE_GLOW;
  c2d.lineWidth = 1.5;
  c2d.shadowBlur = 0;
  c2d.beginPath();
  ...
  c2d.fillStyle = hitFlashMs > 0 ? NADIR_HP_FLASH : NADIR_HP_FG;
...
export function drawNadirCubeMines(
  ...
  for (const mine of mines) {
    ...
    const alpha = Math.min(1, lifeFrac * 3) * 0.85;

    c2d.save();
    c2d.globalAlpha = alpha * pulse;

    if (!isLowGraphics) {
      c2d.shadowBlur = 12;
      c2d.shadowColor = NADIR_MINE_GLOW;
    }
...
export function drawNadirCubeTrailSegments(
  ...
  for (const seg of segments) {
    ...
    if (!isLowGraphics) {
      c2d.shadowBlur = 8;
      c2d.shadowColor = NADIR_TRAIL_COLOR;
    }
...
export function drawNadirCubeTurretBolts(
  ...
  for (const bolt of bolts) {
    ...
    if (!isLowGraphics) {
      c2d.shadowBlur = 10;
      c2d.shadowColor = NADIR_BOLT_GLOW;
    }
...
export function drawNadirCubeLinkLasers(
  ...
  for (const ll of lasers) {
    const isWarning = ll.warningMs > 0;
    const alpha = isWarning ? 0.35 + 0.25 * Math.sin(ll.warningMs / 80) : 0.85;

    c2d.save();
    c2d.globalAlpha = alpha;

    if (!isLowGraphics && !isWarning) {
      c2d.shadowBlur = 14;
      c2d.shadowColor = NADIR_LASER_COLOR;
    }

    c2d.strokeStyle = isWarning ? NADIR_WARN_COLOR : NADIR_LASER_COLOR;
    c2d.lineWidth = isWarning ? 2 : 5;
    c2d.lineCap = 'round';
    c2d.beginPath();
