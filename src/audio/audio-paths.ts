...
import { BOSS_MUSIC_TRACKS_BY_BPM } from './generated-boss-music-tracks';
...
export function getRandomBossMusicTrack(bpm: number): BossMusicTrack | null {
  ...
  return {
    path: encodePath(`ASSETS/music/BossMusic/${filename}`),
    title: formatBossTrackTitle(filename.split('/').at(-1)!),
  };
}
