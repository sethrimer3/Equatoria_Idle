...
import {
  ENEMY_VULN_RATE, ENEMY_CHILL_RATE, ENEMY_WARP_RATE, ENEMY_GRAV_RATE,
  ENEMY_BURN_DPS, ENEMY_POISON_DPS, ENEMY_ECHO_RATE, ENEMY_RIFT_RATE,
  ENEMY_VULN_CAP, ENEMY_CHILL_CAP, ENEMY_WARP_CAP, ENEMY_GRAV_CAP,
  ENEMY_TOTAL_SLOW_CAP, ENEMY_MIN_SPEED, ENEMY_ECHO_CAP,
  ENEMY_RIFT_CAP_PER_STACK, ENEMY_RIFT_STACK_CAP, ENEMY_RIFT_STACK_CAP_BOSS,
  ENEMY_FROZEN_SPEED,
  ENEMY_FRAC_TICKS, ENEMY_FRAC_TICKS_BOSS, ENEMY_FRAC_DECAY, ENEMY_FRAC_MAX_CONCURRENT,
...
interface EnemyStatusState {
  ...
  /** Stack cap for rift-scarred — lower for bosses (set at first riftScarred apply). */
  riftScarredStackCap: number;
}
...
function _getOrCreate(enemy: object): EnemyStatusState {
  ...
  if (!s) {
    s = { statuses: [], riftScarredStacks: new Map(), riftScarredStackCap: ENEMY_RIFT_STACK_CAP };
    _registry.set(enemy, s);
  }
  return s;
}
...
export interface LensStatusParams {
  ...
  /** For Fractal Wound: override tick count (e.g. ENEMY_FRAC_TICKS_BOSS for bosses) */
  fractalTickCount?: number;
  /** For Echo-Marked: damage to echo */
  echoDamage?: number;
  /** For Rift-Scarred: override stack cap (e.g. ENEMY_RIFT_STACK_CAP_BOSS for bosses) */
  riftScarredStackCap?: number;
}
...
export function applyLensStatus(enemy: object, params: LensStatusParams): void {
  ...
  if (params.key === 'fractalWound') {
    // Fractal Wound: always add a new independent wound (up to a max of FRAC_MAX_CONCURRENT active)
    const active = state.statuses.filter(s => s.key === 'fractalWound');
    if (active.length < ENEMY_FRAC_MAX_CONCURRENT) {
      const tickCount = params.fractalTickCount ?? ENEMY_FRAC_TICKS;
      state.statuses.push({
        key: 'fractalWound',
  ...
  if (params.key === 'riftScarred') {
    ...
    // Apply boss/elite stack cap override if provided.
    if (params.riftScarredStackCap !== undefined) {
      state.riftScarredStackCap = params.riftScarredStackCap;
    }
    if (existing) {
...
export function getIncomingDamageMult(enemy: object): number {
  ...
  for (const s of state.statuses) {
    switch (s.key) {
      case 'abraded':   mult *= 1 + clamp(s.magnitude * ENEMY_VULN_RATE, 0, ENEMY_VULN_CAP); break;
      case 'refracted': mult *= 1 + clamp(s.magnitude * ENEMY_VULN_RATE, 0, ENEMY_VULN_CAP); break;
      case 'radiant':   mult *= 1 + clamp(s.magnitude * ENEMY_VULN_RATE, 0, ENEMY_VULN_CAP); break;
      case 'cracked':   mult *= 1 + clamp(s.magnitude * ENEMY_VULN_RATE, 0, ENEMY_VULN_CAP); break;
...
export function getRiftScarredDamageMult(enemy: object, sourceKey: string): number {
  ...
  const bonusPerStack = clamp(magn * ENEMY_RIFT_RATE, 0, ENEMY_RIFT_CAP_PER_STACK);
...
export function incrementRiftScarredStacks(enemy: object, sourceKey: string): void {
  ...
  // Use per-enemy stack cap (lower for bosses, set at riftScarred apply time).
  state.riftScarredStacks.set(sourceKey, Math.min(state.riftScarredStackCap, cur + 1));
}

// ── Movement slow multiplier ───────────────────────────────────────────────────

/** Returns effective speed multiplier for this enemy (ENEMY_FROZEN_SPEED–1.0). */
export function getMovementSlowMult(enemy: object): number {
  const state = _registry.get(enemy);
  if (!state) return 1;
  // Frozen overrides all other slows — nearly immobilises the enemy.
  if (state.statuses.some(s => s.key === 'frozen')) return ENEMY_FROZEN_SPEED;
  let totalSlow = 0;
  for (const s of state.statuses) {
    switch (s.key) {
      case 'chilled':    totalSlow += clamp(s.magnitude * ENEMY_CHILL_RATE, 0, ENEMY_CHILL_CAP); break;
      case 'timeWarped': totalSlow += clamp(s.magnitude * ENEMY_WARP_RATE,  0, ENEMY_WARP_CAP);  break;
      case 'gravitized': totalSlow += clamp(s.magnitude * ENEMY_GRAV_RATE,  0, ENEMY_GRAV_CAP);  break;
    }
  }
  if (totalSlow === 0) return 1;
  return Math.max(ENEMY_MIN_SPEED, 1 - clamp(totalSlow, 0, ENEMY_TOTAL_SLOW_CAP));
}

// ── Echo fraction helper ───────────────────────────────────────────────────────

export function getEchoFraction(magnitude: number): number {
  return clamp(magnitude * ENEMY_ECHO_RATE, 0, ENEMY_ECHO_CAP);
}

// ── Burn/Poison DPS helpers ────────────────────────────────────────────────────

function _burnDmgPerTick(magnitude: number, tickMs: number): number {
  return clamp((magnitude * ENEMY_BURN_DPS) * (tickMs / 1000), 0, 500);
}

function _poisonDmgPerTick(magnitude: number, tickMs: number): number {
  return clamp((magnitude * ENEMY_POISON_DPS) * (tickMs / 1000), 0, 300);
}
      ...
      while (s.tickMs >= s.tickEveryMs) {
        ...
        if (s.key === 'burning') {
          ...
          if ((s.fractalTicksLeft ?? 0) > 0 && s.fractalTickDamage !== undefined) {
            ...
            s.fractalTickDamage = clamp(s.fractalTickDamage * ENEMY_FRAC_DECAY, 0, 5000);
...
/** Returns the active Rift-Scarred stack cap for this enemy. */
